...
registerTool({
  ...
  mode: 'drag',
  properties: [
    { key: 'brushSize', label: 'Eraser Size', type: 'number', min: 1, max: 64, step: 1, default: 1 },
    {
      key: 'brushShape',
      label: 'Eraser Shape',
      type: 'select',
  ...
  onPointerDown(event, ctx) {
    ...
    drawBrush(ctx.ctx, event.x, event.y, size, shape, '', true);
    ctx.refreshTexture();
  ...
  onPointerMove(event, ctx) {
    ...
    let cx = lastX,
      cy = lastY;
    while (true) {
      drawBrush(ctx.ctx, cx, cy, size, shape, '', true);
