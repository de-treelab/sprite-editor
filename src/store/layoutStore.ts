...
export const useLayoutStore = create<LayoutState>((set, get) => ({
  ...
  moveView: (viewId, targetPanelId, anchor) => set(state => {
    // Remove from current location (tree + floating)
    let root = removeViewFromTree(state.layout.root, viewId);
    const floating = state.layout.floating.filter(
      f => !(f.viewIds.includes(viewId) && f.viewIds.length === 1)
    ).map(f => {
  ...
  addView: (viewId, targetPanelId, anchor) => set(state => {
  ...
  setActiveTab: (panelId, viewId) => set(state => {
  ...
  setSplitSizes: (splitId, sizes) => set(state => {
    const updateNode = (node: typeof state.layout.root): typeof state.layout.root => {
      if (node.type === 'split') {
  ...
  floatView: (viewId, position) => set(state => {
  ...
  dockFloating: (floatingPanelId, targetPanelId, anchor) => set(state => {
  ...
  moveFloating: (panelId, position) => set(state => ({
    layout: {
      ...state.layout,
  ...
  resizeFloating: (panelId, size) => set(state => ({
    layout: {
      ...state.layout,
      floating: state.layout.floating.map(f =>
  ...
  bringFloatingToFront: (panelId) => set(state => ({
    layout: {
      ...state.layout,
      floating: state.layout.floating.map(f =>
        f.panelId === panelId ? { ...f, zIndex: ++floatingZCounter } : f
      ),
    },
  })),

  toggleViewHidden: (viewId) => set(state => {
    const hidden = state.layout.hiddenViewIds;
    const hiddenViewIds = hidden.includes(viewId)
      ? hidden.filter(id => id !== viewId)
      : [...hidden, viewId];
    return { layout: { ...state.layout, hiddenViewIds } };
  }),

  setFullscreenView: (viewId) => set(state => ({
  ...
  overwriteLayout: (id) => {
    ...
    const savedLayouts = state.savedLayouts.map(s =>
      s.id === id
        ? { ...s, layout: deepClone(state.layout), updatedAt: Date.now() }
        : s
    );
    saveJson(STORAGE_KEY_LAYOUTS, savedLayouts);
    set({ savedLayouts, activeLayoutId: id });
