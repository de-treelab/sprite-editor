...
export const MenuItem: React.FC<MenuItemProps> = ({
  ...
  return (
    <button className={`${baseStyles} ${stateStyles} ${className}`} onClick={onClick} disabled={disabled}>
      {Icon && <Icon className="mr-2 w-4 h-4" />}
      <span className="flex-1">{label}</span>
      {shortcut && <span className="ml-4 text-xs text-slate-500">{shortcut}</span>}
    </button>
  );
};

// Menu divider
export const MenuDivider: React.FC = () => <div className="h-px bg-slate-700 my-1" />;
